  ...
  return (
    <header className="grid gap-2 sm:grid-cols-[1fr_auto] sm:items-start">
      <div className="min-w-0">
        <h1 className="font-mono text-[1.25rem] font-semibold leading-none tracking-[-0.01em] text-brand-text">
          <span className="text-brand-accent">&gt;</span> {scope}
        </h1>
        {description ? <p className="mt-1 text-[0.75rem] text-brand-textMuted">{description}</p> : null}
        {metrics.length > 0 ? (
          <p className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 font-mono text-[0.6875rem] text-brand-textMuted">
            {metrics.map((metric) => (
      ...
      <div className="flex flex-wrap items-center justify-start gap-2 sm:justify-end">
        <p className="font-mono text-[0.625rem] uppercase tracking-[0.12em] text-brand-textMuted">status: ready</p>
        {action}
      </div>
    </header>
  );
