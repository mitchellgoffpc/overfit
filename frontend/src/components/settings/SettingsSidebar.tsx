...
export default function SettingsSidebar({ sectionLabel, stats }: SettingsSidebarProps): ReactElement {
  ...
  return (
    <aside className="border-b border-[#d2dfdf] px-5 py-5 lg:border-b-0 lg:border-r lg:border-[#d2dfdf] lg:pl-14 lg:pr-5 lg:pt-8">
      ...
      <p className="font-mono text-[0.625rem] uppercase tracking-[0.16em] mt-7 text-brand-textMuted" style={{ lineHeight: "1.875rem" }}>{sectionLabel}</p>
      {stats.map((stat) => (
        <div className="flex items-center justify-between text-[0.75rem]" style={{ height: "1.875rem" }} key={stat.label}>
