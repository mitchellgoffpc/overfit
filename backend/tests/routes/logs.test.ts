...
describe("logs routes", () => {
  ...
  it("reads buffered log deltas with cursor polling", async () => {
    ...
    const firstPage = await request(app)
      ...
      .query({ workerId: "worker-1", count: "2" }).expect(200);
    ...
    const secondPage = await request(app)
      .get(`${API_BASE}/accounts/ada/projects/underfit/runs/run-1/logs`)
      .query({ workerId: "worker-1", cursor: "2" }).expect(200);
  ...
  it("reads persisted segments first and buffered tail on the next cursor", async () => {
    ...
    const persistedPage = await request(app)
      .get(`${API_BASE}/accounts/ada/projects/underfit/runs/run-1/logs`)
      .query({ workerId: "worker-1", cursor: "1", count: "3" }).expect(200);
    ...
    const bufferedPage = await request(app)
      .get(`${API_BASE}/accounts/ada/projects/underfit/runs/run-1/logs`)
      .query({ workerId: "worker-1", cursor: "2", count: "3" }).expect(200);
